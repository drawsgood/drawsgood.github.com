...
module.exports = function(grunt){
  ...
  grunt.initConfig({
      dir: './_deploy'
    , gitUrl: 'git@github.com:drawsgood/drawsgood.github.com.git'
  })
  ...
  grunt.registerTask('deploy', 'Deploy content to github', function () {
    ...
    function gotLastCommit(err, stdout) {
      ...
      if (process.env.NODE_ENV === 'travis-ci') {
        ...
        dirExec('git config user.email "travis-ci@drawsgood.com" && git config user.name "Travis CI Bot"', makeCommit.bind(null, stdout))
      } else {
        makeCommit(stdout)
      }
    }
